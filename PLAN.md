...
1. **Persistent agent:** Cloudflare `AIChatAgent`, one named conversation, native Codex app-server running in a warm Sandbox. Implemented.
...
Keep the public contract to history, send, resume, and cancel. One HTTP Send submits text; the DO starts or steers, and the browser attaches to output separately through AI SDK resume. No custom frontend hook, protocol parser, or browser connection to Cloudflare. No durable state in ephemeral PL-style servers. See README.md for setup and explicit prototype limits.
...
See [the direct Codex sandbox plan](docs/codex-sandbox-plan.md). Implemented locally; live container acceptance remains user-run. It keeps the existing chat coordinator and runs the full native Codex harness through app-server inside Cloudflare Sandbox. Coordinator restart stops work and reports interruption; it does not automatically replay the prompt. The OpenAI Agents API is explicitly excluded.

Sandbox lifecycle follows the waiting-state subset of [Course agent MVP #15681](https://github.com/PrairieLearn/PrairieLearn/issues/15681): ten minutes waiting for the user triggers backup then destruction; six hours without accepted user interaction triggers bounded interruption/checkpoint and destruction. Cleanup retries are bounded. Approval/publishing states remain outside this prototype.
